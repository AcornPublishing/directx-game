...
bool CDXGObject::LoadSkinMeshFromFile(char* filename)
{
	...
	CAnimationSet *tempAnimSet = AnimationSets;
	while(tempAnimSet != NULL) {
		CAnimation *tempAnim = tempAnimSet->Animations;
		while(tempAnim != NULL) {
			tempAnim->Bone = FindDXFrame(tempAnim->sName, SkinFrame);
			tempAnim = tempAnim->NextAnim;
		}
		tempAnimSet = tempAnimSet->NextAnimSet;
	}
	...
	IDirectXFileData *pParentDataObj,  D3DXFRAME_DERIVED **frd, bool ref)
{
	...
	// ============== Frame ============== //
	if (*Type == TID_D3DRMFrame && AnimationSets) 
	{
...
void CDXGObject::UpdateSkinAnimation(DWORD Time)
{
	...
	while (Anim) 
	{
		...
		if (Anim->numKeys && Anim->animationKeys) 
		{
			...
			D3DXMATRIX newMatrix = (Anim->animationKeys[Key2].m_matKey - Anim->animationKeys[Key1].m_matKey)*(Time - Anim->animationKeys[Key1].Time) / (Anim->animationKeys[Key2].Time - Anim->animationKeys[Key1].Time);
