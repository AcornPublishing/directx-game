...
void CDXGObject::LoadTriangles()
{
  ...
  WORD *pInd = NULL;
  pObjMesh->LockVertexBuffer( D3DLOCK_READONLY, reinterpret_cast<VOID**>(&pVerts) );
  pObjMesh->LockIndexBuffer( D3DLOCK_READONLY, reinterpret_cast<VOID**>(&pInd) );
  for( DWORD dwFace = 0; dwFace < m_dwNumTriangles; dwFace++ )
  {
    ...
    D3DXVec3Normalize( &(m_pTriangles[dwFace].N), &(m_pTriangles[dwFace].N) );

    m_pTriangles[dwFace].d = -D3DXVec3Dot( &(m_pTriangles[dwFace].N), &(m_pTriangles[dwFace].v0) );
  } 
...
void CDXGObject::GetFace(DWORD dwFace, TRIANGLE &triangle)
{
  if(m_pTriangles && dwFace < m_dwNumTriangles)
  {
