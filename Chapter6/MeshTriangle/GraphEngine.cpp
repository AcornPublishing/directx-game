...
bool CGraphEngine::InitObject(int type, int index)
{
	...
	switch(type)
	{
		case SO_DEFAULT_ROOM:
			objects[index]->LoadMeshFromFile("Media\\room.x", "wall.bmp");
...
/*	for (i=0; i<objects[0].GetPlaneNum(); i++)
	{
		TRIANGLE ct;
		objects[0].GetFace(i, &ct);
		D3DXPLANE(
		D3DXVECTOR3 = ct.v0
        
		float Dist = D3DXVec3Dot() + 
	}*/
