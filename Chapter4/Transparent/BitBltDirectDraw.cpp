...
WORD baImageBits[180][630];
WORD cImageTransparent;
                     ...
                     int       nCmdShow)
{
	...
	if (plainsurf->Lock(0, &desc, DDLOCK_WAIT, 0)==DD_OK)
	{
        BYTE* dst = (BYTE *)desc.lpSurface;

		for (int x=0; x<180; x++)
            for (int y=0; y<630; y++)
				baImageBits[x][y]=*(WORD *)(dst+y*desc.lPitch+x*2);
	...
	while (true) 
	{
		...
		if (imagesurf!=NULL)
		{
			...
			if (backsurf->Lock(&dst, &desc, DDLOCK_WAIT, 0)==DD_OK)
			{				                
				// Transparent copy
				for (int j=src.top; j<src.bottom; j++)
				{
					for (int i=src.left; i<src.right; i++)
					{
						...
						BYTE* wrkPointer = (BYTE *)desc.lpSurface + (j-src.top) * desc.lPitch + (i-src.left) * 2;

						WORD old_blue = *(WORD *)wrkPointer & 0x1f;
						WORD old_green = (*(WORD *)wrkPointer >> 5)  & 0x3f;
						WORD old_red = (*(WORD *)wrkPointer >> 11) & 0x1f;

						WORD blue  = old_blue+(alpha * ((baImageBits[i][j] & 0x1f) - old_blue) >> 8);
						WORD green = old_green+(alpha * (((baImageBits[i][j] >> 5)  & 0x3f) - old_green) >> 8);
						WORD red   = old_red+(alpha * (((baImageBits[i][j] >> 11) & 0x1f) - old_red) >> 8);
