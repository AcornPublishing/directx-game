                     ...
                     int       nCmdShow)
{
	...
	while (true) 
	{
		...
		if (zoomsurf->Lock(0, &desc, DDLOCK_WAIT, 0)==DD_OK)
		{
			BYTE* dst = (BYTE *)desc.lpSurface;
			for (int i=0; i<200; i++)
				for (int j=0; j<200; j++)
                    if (((i - 100)*(i - 100)) + ((j - 100)*(j - 100)) > 100 * 100)
                        *(WORD *)(dst + j * desc.lPitch + i * 2) = 0;
			zoomsurf->Unlock(0);
		}

		backsurf->BltFast(xPos-100, yPos-100, zoomsurf, 0, 
