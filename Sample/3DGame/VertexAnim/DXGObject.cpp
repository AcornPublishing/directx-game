...
bool CDXGObject::Render(DWORD ef_index, int light)
{
	...
	if (pMeshKey1)
	{
		...
		MeshVertex *pObj = NULL;
		MeshVertex *pKey1 = NULL;
		MeshVertex *pDraw = NULL;
		...
		for (DWORD dw = 0; dw < pObjMesh->GetNumVertices(); dw++)
			pDraw[dw].vPosition = pKey1[dw].vPosition*t+pObj[dw].vPosition*(1 - t);
...
void CDXGObject::LoadMeshFromFile(char* filename, char* texturename)
{
	...
	pObjMesh->CloneMesh(0, (const D3DVERTEXELEMENT9 *)&decl, pDevice, &pMeshToDraw);
}
