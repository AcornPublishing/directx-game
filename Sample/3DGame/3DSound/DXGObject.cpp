...
bool CDXGObject::Render(DWORD ef_index, int light)
{
	...
	if (pMeshKey1)
	{
        ...
        if ((tCurrentTime-tStartAnimTime)>4000)
			tStartAnimTime = GetTickCount();
		float t = (float)(tCurrentTime - tStartAnimTime) / 4000;

		if (pEffect[ef_index]->IsParameterUsed("mTime", pTechniqueName[ef_index]))
	...
	for (UINT i = 0; i < uPass; i++)
    {
		...
		if (pMeshTextures[0])
		{
			...
			if (pEffect[ef_index]->IsParameterUsed("tText", pTechniqueName[ef_index]))
				pEffect[ef_index]->SetValue("tText", &pMeshTextures[0], D3DX_DEFAULT);
		}
		...
		pDevice->SetStreamSource(0, pVB, 0, D3DXGetFVFVertexSize(pObjMesh->GetFVF()));
		pDevice->SetIndices(pIB);

		if (pMeshKey1)
		{
			pMeshKey1->GetVertexBuffer(&pVBKey);
			pDevice->SetStreamSource(1, pVBKey, 0, D3DXGetFVFVertexSize(pMeshKey1->GetFVF()));

			pDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0, numVertices, 0, numFaces);
...
void CDXGObject::LoadMeshFromFile(char* filename, char* texturename)
{
	...
	pObjMesh->CloneMesh(0, (const D3DVERTEXELEMENT9 *)&decl, pDevice, &pObjMesh);
}
...
void CDXGObject::SkinnedMeshPositioning()
{
	...
	r = FindDXFrame("Bip01_R_UpperArm", SkinFrame);
	D3DXMatrixRotationX(&r->TransformationMatrix, 2.5f);
...
void CDXGObject::getMeshBoundBox()
{
	BYTE* pVertices;
    if (FAILED(pObjMesh->LockVertexBuffer(D3DLOCK_READONLY, (LPVOID*)&pVertices)))
        return;
 
	D3DXComputeBoundingBox((D3DXVECTOR3*)pVertices, pObjMesh->GetNumVertices(), 
        D3DXGetFVFVertexSize(pObjMesh->GetFVF()), &vecBoundingBoxMin, &vecBoundingBoxMax);
...
void CDXGObject::LoadKeyFromFile(char* filename, char* texturename)
{
	...
	pMeshKey1->CloneMesh(0, (const D3DVERTEXELEMENT9 *)&decl, pDevice, &pMeshKey1);
}
