...
CGraphEngine::CGraphEngine(IDirect3DDevice9 *pD3DDevice, HWND hw)
{
	hWnd = hw;
	pDevice=pD3DDevice;
	...
	// Anit audio and create path
	pPerformance->InitAudio(NULL, NULL, hWnd, DMUS_APATH_DYNAMIC_STEREO, 128, DMUS_AUDIOF_ALL, NULL);

	if (FAILED(pPerformance->CreateStandardAudioPath(DMUS_APATH_DYNAMIC_3D, 64, TRUE, &pAudioPath)))
        pAudioPath = NULL;
    ...
    pPerformance->PlaySegmentEx(pSegment, NULL, NULL, DMUS_SEGF_BEAT, 0, NULL, NULL, pAudioPath);
