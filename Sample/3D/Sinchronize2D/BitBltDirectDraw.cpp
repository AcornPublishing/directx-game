...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   imagesurf2 = LoadBMPToSurface("plain.bmp", ppiDD);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		break;
	case WM_DESTROY:
		 if (imagesurf2) {imagesurf2->Release();imagesurf2=0;}
		 if (imagesurf) {imagesurf->Release();imagesurf=0;}
...
void DrawScene()
{
	...
	double iPos=StartPos+(EndPos-StartPos)*((tCurrentTime-tStartAnimTime)/AnimTime);

	// 두번째 이미지 표시
	backsurf->BltFast(iPos, 100, imagesurf2, 0, DDBLTFAST_SRCCOLORKEY | DDBLTFAST_WAIT);
