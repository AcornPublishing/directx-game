...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   D3DXMatrixTranslation(&mView, 0.0f, 0.0f, 7.0f);
...
void DrawScene()
{
    ...
    D3DXMatrixTranslation(&mat, 0.0f, -1.0f, 3.0f);
    ...
    for (int i=0; i<6; i++)
    {
        ...
        RenderWorld(&mView, &matProj);
    }
    pRenderToEnvMap->End(0);

	// Render scene
    RenderWorld(&mView, &mProject);

	// Render mirror
	LPDIRECT3DVERTEXBUFFER9 pVB;
	LPDIRECT3DINDEXBUFFER9 pIB;

    pTube->GetVertexBuffer(&pVB);
	pTube->GetIndexBuffer(&pIB);
	...
	pD3DDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0,
			0, pTube->GetNumVertices(), 0, pTube->GetNumFaces());
